			...
			<tbody>
				<c:forEach var="order" items="${listOrder}">
					<tr>
						...
						<td><c:if test="${order.getCoverImage() == null}">
								<img class="img-fluid"
									style="width: 40px; height: 40px;"
									src="http://res.cloudinary.com/multichoice/image/upload/v1630841637/lfwmtioizab0976ebcna.png">
							</c:if> <c:if test="${order.getCoverImage() != null}">
								<img class="rounded-circle img-fluid"
									style="width: 40px; height: 40px;"
									src="<c:out value="${order.getCoverImage()}" />" alt="...">
							</c:if></td>
						<td><c:out value="${order.getPrice()}" /></td>
						<td><c:out
								value="${order.getBook().getTitle()} - ${order.getBook().getAuthor()}" /></td>
						<td>
							<div>
								<a class="btn btn-outline-primary"
									...
									href="<%=request.getContextPath()%>/admin/item-book/delete?id=<c:out value='${ .getID()}' />"><i
